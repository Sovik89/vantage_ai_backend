...
HR_PA_CONTENT_CLASSIFIER_PROMPT = """
You are an expert content validator for 'Vantage.Ai', specializing in both HR and People Analytics. 
Your task is to determine if content belongs in our professional knowledge base.
Analyze the text and respond with a single word: ACCEPT or REJECT.

**ACCEPT if the content primarily focuses on:**
...
3. Strategic HR & Organizational Topics:
   * Employee engagement
...
**REJECT if the content primarily focuses on:**
* General business (without HR/people focus)
* Pure technical content (without HR context)
* Consumer/personal topics
* Academic research (unless specifically about workplace/HR)
* General social science (unless workplace-focused)

Text to evaluate: "{snippet}"
"""

# # Update the import section if needed:
# # ...existing code...
# HR_TEXT_CLASSIFIER_PROMPT = HR_PA_CONTENT_CLASSIFIER_PROMPT  # Replace or add new variable
# # ...existing code...
...
# ✅ OPTION A: MORE FLEXIBLE VERSION (allows creative HR-related questions)
HR_QUERY_CLASSIFIER_PROMPT = """
You are an AI assistant that specializes in Human Resources (HR) and People Analytics.
Determine if this query is related to HR, People Analytics, or the workplace.

**ACCEPT (return YES) if the query is about:**
- Human Resources topics: recruitment, retention, performance, compensation, benefits
- People Analytics: workforce data, metrics, trends, predictions
- Workplace topics: culture, leadership, teams, organizational behavior
- Employee experience: engagement, satisfaction, development, wellbeing
- Labor & employment: regulations, compliance, labor economics, future of work
- DEI topics: diversity, equity, inclusion, discrimination, bias
- Management & operations: organizational design, change management
- Technology in HR: HRIS, AI in recruitment, workforce automation
- Creative or hypothetical questions that ultimately relate to workplace/HR context
- Meta-questions about exporting, formatting, or saving content

**REJECT (return NO) if the query is:**
- Completely unrelated to work or organizations (e.g., cooking recipes, sports trivia)
- About education/schools (unless specifically about workplace training)
- About healthcare/medicine (unless specifically about employee wellness)
- Pure technical questions unrelated to HR context

**Examples:**
- "What is employee retention?" → YES (core HR topic)
- "How will AI impact HR in 2030?" → YES (future of work)
- "What are quantum mechanics?" → NO (not work-related)
- "How will quantum mechanics impact HR policies?" → YES (creative but has HR context)
- "What's the weather today?" → NO (unrelated)
- "How to make pizza?" → NO (unrelated)
...
# ✅ OPTION B: STRICT VERSION (original - better for quality control)
# HR_QUERY_CLASSIFIER_PROMPT = """
# You are an AI assistant that specializes in Human Resources (HR) and People Analytics.
...
SENTIMENT_PROMPTS = {
    "exit_feedback": """You are an HR Sentiment Analysis expert.

IMPORTANT: Your response must be ONLY a raw JSON object. Do NOT wrap it in markdown code blocks. Do NOT use ```json tags. Do NOT add any explanation before or after the JSON.

Classify the overall sentiment of the following employee exit feedback and return this exact JSON structure:
{{
...
Text: {text}""",
    ...
    "interview_feedback": """You are an HR Analyst evaluating interview feedback.
...
Text: {text}""",
    ...
    "appraisal_feedback": """You are a People Analytics assistant analyzing performance appraisal comments.
...
Text: {text}""",
    ...
    "pulse_feedback": """You are an organizational sentiment classifier analyzing pulse survey responses.
...
Text: {text}""",
    ...
    "training_feedback": """You are an HR sentiment classifier analyzing employee training feedback.
...
Text: {text}""",
    ...
    "onboarding_feedback": """You are an HR classifier analyzing onboarding experience feedback.
...
Text: {text}""",
    ...
    "manager_feedback": """You are a People Analytics classifier analyzing manager feedback.
...
{{
 ...
 "confidence": 0.0-1.0,
 "reason": "one sentence reason",
 "topic": "leadership|communication|support|fairness|other"
}}

Text: {text}""",

    "employee_feedback": """You are an HR sentiment analyst reviewing general employee feedback.
...
{{
 ...
 "confidence": 0.0-1.0,
 "reason": "one sentence reason",
 "category": "work_environment|benefits|recognition|career|culture|other"
}}

Text: {text}""",

    "general": """You are a general sentiment classifier for workplace feedback.
